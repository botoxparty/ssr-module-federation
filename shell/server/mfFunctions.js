...
const mfAppNames = ['remote1'].join('|');
const mfAppNamesRegex = RegExp(`(${mfAppNames})-.*`);
const mfStatsUrlMap = {
  remote1: 'http://localhost:3002/static/federation-stats.json',
};
...
export const getMfChunks = async extractor => {
  ...
  console.log({ loadableRequiredComponents });
  ...
  console.log({ mfRenderedComponents });
